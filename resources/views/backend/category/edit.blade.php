        ...
        <div class="row">
          <!-- left column -->
            ...
            <div class="card card-primary">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form id="form" method="POST" action="{{ route('backend.posts.categories.update', $cat->id) }}">
                    @method('PUT')
                    @csrf
                  ...
                  <div class="card-body">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" value="{{ $cat->name }}" id="name" name="name" onkeyup="getSlug()" onchange="getSlug()" placeholder="Nhập tên chuyên mục">
                    </div>
                    
                    <div class="form-group">
                    <label class="text-danger">{{ $errors->first('name_route') }}</label></br>
                      <label for="">Đường dẫn chuyên mục</label>
                      <input type="text" class="form-control" value="{{ $cat->name_route }}" id="name_route" name="name_route" placeholder="Nhập đường dẫn chuyên mục">
                    </div>
                    <div class="form-group">
                    <label class="text-danger">{{ $errors->first('parent_id') }}</label></br>
                        ...
                        <select class="custom-select rounded-0" id="parent_id" name="parent_id">
                            ...
                            <option value="{{ $category->id }}" {{ $cat->parent_id == $category->id ? "selected" : "" }} >{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>
					<div class="form-group">
                    <label class="text-danger">{{ $errors->first('status') }}</label></br>
                        <div class="custom-control custom-checkbox">
							<input class="custom-control-input" value="1" name="status" type="checkbox" id="customCheckbox1" {{ $cat->status == 1 ? "checked" : "" }}>
