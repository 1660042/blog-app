                    ...
                    <div class="card-body table-responsive p-0" style="height: auto;">
                        ...
                        @foreach ($posts as $post)
                         ...
                         <tr class="">
                          ...
                          <td class="text-center align-middle {{ $post->status == '1' ? "" : "text-danger" }}">{{ $status[$post->status] }}</td>
                          <td class="text-center align-middle">{{ $post->createdBy == null ? "" : $post->createdBy->name }}</td>
                          <td class="text-center align-middle">{{ $post->updatedBy == null ? "" : $post->updatedBy->name }}</td>
                          <td class="text-center align-middle">
                             <div class="">
                                <!-- <div class="col-sm-6"> -->
                                <a href="{{ route('backend.posts.posts.edit', $post->id) }}" class="btn btn-primary btn-sm" title="Sửa bài viết"><i class="far fa-edit" ></i>&ensp;Sửa</a>
                                <!-- </div>
                                <div class="col-sm-6"> -->
                                  <button type="button" title="Xóa bài viết" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modal-overlay-{{$post->id}}">
              ...
              <div class="modal-dialog">
                <div class="modal-content" id="modal-content-{{ $post->id }}">
                  ...
                  <div class="modal-footer justify-content-between">
                    ...
                    <button type="button" onclick="deletePost({{ $post->id }})" class="btn btn-danger">Xóa</button>
                  </div>
                  <form id="form-delete-{{ $post->id }}" method="POST" action="{{ route('backend.posts.posts.delete', $post->id) }}">
                    @csrf
                    @method('DELETE')
                  </form>
                </div>
